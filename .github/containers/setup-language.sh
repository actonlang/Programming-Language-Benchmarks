...
case "$language" in
  acton)
    ...
    ;;
  c|cpp|chapel|dart|elixir|go|hacklang|haskell|java|kotlin|pony|python|swift)
    ;;
  codon)
  ...
  hare)
    ...
    cp /tmp/harec/config.example.mk /tmp/harec/config.mk
    make -C /tmp/harec -j2
    make -C /tmp/harec install PREFIX=/usr/local
    git clone --depth 1 https://git.sr.ht/~sircmpwn/hare /tmp/hare
    cp /tmp/hare/config.example.mk /tmp/hare/config.mk
    make -C /tmp/hare -j2
  ...
  wasm)
    ...
    curl --fail --location --retry 3 https://wasmtime.dev/install.sh -o /tmp/wasmtime.sh
    bash /tmp/wasmtime.sh
    ;;
  wren)
